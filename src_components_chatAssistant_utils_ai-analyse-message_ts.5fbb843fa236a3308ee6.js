...
async function handleAudioToSummaryText({
  ...
  try {
    ...
    if (size && size > 4.5 * 1024 * 1024) {
      antd__WEBPACK_IMPORTED_MODULE_7__["default"].info("File size exceeds limit (4.5 MB)");
      newMessage = {
        ...newMessage,
        content: JSON.stringify({
          message,
...
function getAutoTransLang() {
  ...
  return new Intl.DisplayNames([autoTranslateLanguage], {
    type: "language"
  }).of(autoTranslateLanguage) || "en";
}
...
//# sourceMappingURL=src_components_chatAssistant_utils_ai-analyse-message_ts.5fbb843fa236a3308ee6.js.map