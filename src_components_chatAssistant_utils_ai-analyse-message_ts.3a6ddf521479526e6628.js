...
/* harmony export */ });
/* harmony import */ var _lib_EventEmitter__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ../lib/EventEmitter */ "./src/components/chatAssistant/lib/EventEmitter.ts");
/* harmony import */ var _store__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../store */ "./src/components/chatAssistant/store/index.ts");
/* harmony import */ var _chat_api__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./chat-api */ "./src/components/chatAssistant/utils/chat-api.ts");
/* harmony import */ var _ai_chatfolders_util__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../ai-chatfolders/util */ "./src/components/chatAssistant/ai-chatfolders/util.ts");
...
/* harmony import */ var pdfjs_dist_legacy_build_pdf__WEBPACK_IMPORTED_MODULE_7__ = __webpack_require__(/*! pdfjs-dist/legacy/build/pdf */ "./node_modules/pdfjs-dist/legacy/build/pdf.js");
/* harmony import */ var pdfjs_dist_legacy_build_pdf__WEBPACK_IMPORTED_MODULE_7___default = /*#__PURE__*/__webpack_require__.n(pdfjs_dist_legacy_build_pdf__WEBPACK_IMPORTED_MODULE_7__);
/* harmony import */ var _global__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../../../global */ "./src/global/index.ts");
/* harmony import */ var _global_selectors__WEBPACK_IMPORTED_MODULE_9__ = __webpack_require__(/*! ../../../global/selectors */ "./src/global/selectors/index.ts");
/* harmony import */ var _lib_message_creater__WEBPACK_IMPORTED_MODULE_10__ = __webpack_require__(/*! ../lib/message-creater */ "./src/components/chatAssistant/lib/message-creater.ts");
/* eslint-disable */
...
pdfjs_dist_legacy_build_pdf__WEBPACK_IMPORTED_MODULE_7__.GlobalWorkerOptions.workerSrc = "pdf.worker.min.js";
...
async function replyToMention(message, isAuto = false) {
  ...
  let newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIReplyMentionMessage)({
  ...
  (0,_chat_api__WEBPACK_IMPORTED_MODULE_2__.mentionReply)({
...
async function photoSummary(message, isAuto = false) {
  ...
  if (!photo) return;
  const mediaHash = (0,_global_helpers__WEBPACK_IMPORTED_MODULE_4__.getMediaHash)(photo, "download");
  if (!mediaHash) return;
  const mimeType = "image/jpeg";
  const newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIMediaSummaryMessage)({
    chatId: message.chatId,
    metadata: {
      message,
      isAuto,
...
async function webPageSummary(message, isAuto = false) {
  ...
  const webPage = (0,_global_selectors__WEBPACK_IMPORTED_MODULE_9__.selectWebPageFromMessage)(global, message);
  const url = webPage ? webPage?.url : message.content?.text?.text;
  if (!url || url === "") return;
  let newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIMediaSummaryMessage)({
    chatId: message.chatId,
    metadata: {
      message,
      isAuto,
  ...
  await sendMessageToAIRoom(newMessage);
  (0,_chat_api__WEBPACK_IMPORTED_MODULE_2__.webPageAISummary)({
    url,
...
async function documentSummary(message, isAuto = false) {
  ...
  if (!document) return;
  const mediaHash = (0,_global_helpers__WEBPACK_IMPORTED_MODULE_4__.getMediaHash)(document, "download");
  if (!mediaHash) return;
  ...
  await _util_mediaLoader__WEBPACK_IMPORTED_MODULE_5__.fetch(mediaHash, 0);
  const blobUrl = _util_mediaLoader__WEBPACK_IMPORTED_MODULE_5__.getFromMemory(mediaHash);
  if (!blobUrl) return;
  ...
  switch (document.mimeType) {
    ...
    case "application/pdf":
      const pdf = await pdfjs_dist_legacy_build_pdf__WEBPACK_IMPORTED_MODULE_7__.getDocument({
        data: arrayBuffer
      }).promise;
      let pdfText = "";
  ...
  (0,_chat_api__WEBPACK_IMPORTED_MODULE_2__.documentAISummary)({
    content: text,
    language: getAutoTransLang()
  }).then(response => {
    if (response?.text) {
...
async function voiceSummary(message, isAuto = false) {
  ...
  if (transcriptionId && transcriptionId !== "") {
    ...
    await (0,_global__WEBPACK_IMPORTED_MODULE_8__.getActions)().transcribeAudio({
      chatId: message.chatId,
      messageId: message.id
    });
    await (0,_ai_chatfolders_util__WEBPACK_IMPORTED_MODULE_3__.sleep)(2000);
    const newGlobal = (0,_global__WEBPACK_IMPORTED_MODULE_8__.getGlobal)();
    const currentMessage = (0,_global_selectors__WEBPACK_IMPORTED_MODULE_9__.selectChatMessage)(newGlobal, message.chatId, message.id);
...
async function voiceToAudioSummary(message, isAuto = false) {
  const voice = message.content?.voice;
  if (!voice) return;
  const mediaHash = (0,_global_helpers__WEBPACK_IMPORTED_MODULE_4__.getMediaHash)(voice, "download");
  if (!mediaHash) return;
  const maxBase64Length = Math.floor(4 * 1024 * 1024);
  if (message.content.audio?.size && message.content.audio?.size * (4 / 3) > maxBase64Length) {
    antd__WEBPACK_IMPORTED_MODULE_6__["default"].info("File size exceeds limit (4 MB)");
    return;
  }
  let newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIMediaSummaryMessage)({
    chatId: message.chatId,
...
async function audioSummary(message, isAuto = false) {
  ...
  if (!audio) return;
  const mediaHash = (0,_global_helpers__WEBPACK_IMPORTED_MODULE_4__.getMediaHash)(audio, "download");
  if (!mediaHash) return;
  let newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIMediaSummaryMessage)({
    chatId: message.chatId,
    metadata: {
...
async function videoSummary(message, isAuto = false) {
  ...
  if (!video) return;
  const mediaHash = (0,_global_helpers__WEBPACK_IMPORTED_MODULE_4__.getMediaHash)(video, "download");
  if (!mediaHash) return;
  let newMessage = (0,_lib_message_creater__WEBPACK_IMPORTED_MODULE_10__.createAIMediaSummaryMessage)({
    chatId: message.chatId,
    metadata: {
...
async function sendMessageToAIRoom(newMessage) {
  await _store__WEBPACK_IMPORTED_MODULE_1__.ChataiStores.message?.storeMessage(newMessage);
  await _lib_EventEmitter__WEBPACK_IMPORTED_MODULE_0__["default"].emit(_lib_EventEmitter__WEBPACK_IMPORTED_MODULE_0__.Actions.AddRoomAIMessage, newMessage);
}
async function handleAudioToSummaryText({
  ...
  try {
    ...
    if (isAuto && size && size > 50 * 1024 * 1024 || !isAuto && size && size > 100 * 1024 * 1024) {
      antd__WEBPACK_IMPORTED_MODULE_6__["default"].info("File too large to summarize");
      newMessage = {
        ...newMessage,
        metadata: {
          message,
    ...
    const summaryResponse = await (0,_chat_api__WEBPACK_IMPORTED_MODULE_2__.audioAISummary)(formData);
...
async function handleImageToSummaryText({
  ...
  reader.onloadend = function () {
    ...
    (0,_chat_api__WEBPACK_IMPORTED_MODULE_2__.imageAISummary)({
...
//# sourceMappingURL=src_components_chatAssistant_utils_ai-analyse-message_ts.3a6ddf521479526e6628.js.map